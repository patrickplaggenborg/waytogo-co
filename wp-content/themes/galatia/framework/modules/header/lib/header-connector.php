...
namespace GalatiaEdgeNamespace\Modules\Header\Lib;

/**
 * Class GalatiaEdgeClassHeaderConnector
 *
...
class GalatiaEdgeClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'galatia_edge_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'galatia_edge_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'galatia_edge_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'galatia_edge_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to galatia_edge_filter_title_content_padding filter
	 *
	 * @return int
	 */
