...
class GalatiaEdgeClassLike {
	private static $instance;

	private function __construct() {
		add_action( 'wp_ajax_galatia_edge_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_galatia_edge_like', array( $this, 'ajax' ) );
	}
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			...
			case 'get':
				...
				$icon         = '';
				$return_value = $icon . "<span>" . esc_attr( $like_count );

				if ( $like_count == 1 ) {
					$return_value .= '<span class="edgtf-like-text">' . esc_html__( 'like', 'galatia' ) . '</span>';
				} else {
					$return_value .= '<span class="edgtf-like-text">' . esc_html__( 'likes', 'galatia' ) . '</span>';
				}
			...
			case 'update':
				...
				$return_value = "<span>" . esc_attr( $like_count );
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'galatia' );

		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'galatia' );
		}
...
if ( ! function_exists( 'galatia_edge_create_like' ) ) {
	function galatia_edge_create_like() {
		GalatiaEdgeClassLike::get_instance();
	}

	add_action( 'after_setup_theme', 'galatia_edge_create_like' );
}