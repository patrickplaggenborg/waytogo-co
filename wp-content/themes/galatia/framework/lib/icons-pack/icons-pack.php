...
foreach ( glob( EDGE_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( galatia_edge_is_customizer_item_enabled( $module_load, 'galatia_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'galatia_edge_get_icon_pack_collections' ) ) {
	function galatia_edge_get_icon_pack_collections() {
		$options = apply_filters( 'galatia_edge_filter_add_icon_pack_into_collection', $options = array() );

		return $options;
	}

	add_action( 'after_setup_theme', 'galatia_edge_get_icon_pack_collections' );
}

if ( ! function_exists( 'galatia_edge_get_icon_pack_options' ) ) {
	function galatia_edge_get_icon_pack_options() {
		$options = apply_filters( 'galatia_edge_filter_add_icon_pack_into_options', $options = array() );

		return $options;
	}

	add_action( 'after_setup_theme', 'galatia_edge_get_icon_pack_options' );
}

if ( ! function_exists( 'galatia_edge_get_icon_pack_social_options' ) ) {
	function galatia_edge_get_icon_pack_social_options() {
		$options = apply_filters( 'galatia_edge_filter_add_icon_pack_into_social_options', $options = array() );

		return $options;
	}

	add_action( 'after_setup_theme', 'galatia_edge_get_icon_pack_social_options' );
}

/*
 * Class: GalatiaEdgeClassIconCollections
 */
...
class GalatiaEdgeClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = galatia_edge_get_icon_pack_collections();
		$this->iconPackOptions       = galatia_edge_get_icon_pack_options();
		$this->iconPackSocialOptions = galatia_edge_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = galatia_edge_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'galatia' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'galatia' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'galatia' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'galatia' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'galatia' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'galatia' ), $n )
			);

			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'galatia' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'galatia' )       => 'fa-lg',
			esc_html__( 'Small', 'galatia' )      => 'fa-2x',
			esc_html__( 'Medium', 'galatia' )     => 'fa-3x',
			esc_html__( 'Large', 'galatia' )      => 'fa-4x',
			esc_html__( 'Very Large', 'galatia' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iGalatiaEdgeInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'galatia' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                       = array();
		$vc_array[ esc_html__( 'No Icon', 'galatia' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'galatia-edge-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
