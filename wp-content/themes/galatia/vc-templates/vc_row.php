...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $background_color_in_grid = $simple_background_image = $background_image_position = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = '';
...
$row_background_color_in_grid = '';

if( ! empty($background_color_in_grid) ){
    $css_classes[] = 'edgtf-row-with-in-grid-bg-color';

    $row_background_color_in_grid .= '<div class="edgtf-in-grid-bg-color-holder">';
    $row_background_color_in_grid .=    '<div class="edgtf-container-inner" style="background-color: ' . $background_color_in_grid . '">';
    $row_background_color_in_grid .=    '</div>';
    $row_background_color_in_grid .= '</div>';
}
...
if ( $row_content_width === 'grid' ) {
	$edgtf_row_wrapper_start .= '<div class="edgtf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . galatia_edge_get_inline_style( $edgtf_grid_row_styles ) . '><div class="edgtf-row-grid-section">';
	$edgtf_row_wrapper_end   .= '</div></div>';
}

$edgtf_after_wrapper_open   = apply_filters('galatia_edge_filter_vc_after_wrapper_open', '', $atts);
$edgtf_before_wrapper_close = apply_filters('galatia_edge_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('galatia_edge_filter_vc_css_classes', $css_classes, $atts);
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . galatia_edge_get_inline_style( $edgtf_vc_row_styles ) . '>';
$output .= $edgtf_after_wrapper_open;
$output .= wpb_js_remove_wpautop( $content );
$output .= $edgtf_before_wrapper_close;
if($row_background_color_in_grid !== '') {
    $output .= $row_background_color_in_grid;
}
$output .= '</div>';
$output .= $edgtf_row_wrapper_end;
$output .= $after_output;

echo galatia_edge_get_module_part( $output );
