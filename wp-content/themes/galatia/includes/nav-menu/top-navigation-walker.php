...
if ( ! class_exists( 'GalatiaEdgeClassTopNavigationWalker' ) ) {
	class GalatiaEdgeClassTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if ( $this->background_image !== '' ) {
				$menu_image_background  = "background-image:url(" . $this->background_image . "); background-repeat: no-repeat; background-position: center center;";
				$background_image_class = ' background_image';
			}

			$indent = str_repeat( "\t", $depth );
			if ( $depth == 0 ) {
				$out_div = '<div class="second ' . $background_image_class . '"><div class="inner" ' . ( $menu_image_background != '' ? galatia_edge_get_inline_style( $menu_image_background ) : '' ) . '>';
			} else {
				$out_div = '';
			}
