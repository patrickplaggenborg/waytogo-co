...
class GalatiaTwitterWidget extends WP_Widget {
	private $params;

	public function __construct() {
		parent::__construct(
			'edgtf_twitter_widget',
			esc_html__( 'Galatia Twitter Widget', 'galatia-twitter-feed' ),
			array(
				'description' => esc_html__( 'Display your Twitter feed', 'galatia-twitter-feed' )
			)
		);
	...
	private function setParams() {
		$this->params = array(
			array(
				'name'  => 'title',
				'type'  => 'textfield',
				'title' => esc_html__( 'Title', 'galatia-twitter-feed' )
			),
			array(
				'name'    => 'type_of_widget',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'galatia-twitter-feed' ),
				'options' => array(
					'standard' => esc_html__( 'Standard', 'galatia-twitter-feed' ),
					'slider'   => esc_html__( 'Slider', 'galatia-twitter-feed' )
				)
			),
			array(
				'name'  => 'user_id',
				'type'  => 'textfield',
				'title' => esc_html__( 'User ID', 'galatia-twitter-feed' )
			),
			array(
				'name'  => 'count',
				'type'  => 'textfield',
				'title' => esc_html__( 'Number of Tweets', 'galatia-twitter-feed' )
			),
			array(
				'name'    => 'show_tweet_icon',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Icon', 'galatia-twitter-feed' ),
				'options' => array(
					'yes' => esc_html__( 'Yes', 'galatia-twitter-feed' ),
					'no'  => esc_html__( 'No', 'galatia-twitter-feed' )
				)
			),
			array(
				'name'    => 'show_tweet_time',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Time', 'galatia-twitter-feed' ),
				'options' => array(
					'no'  => esc_html__( 'No', 'galatia-twitter-feed' ),
					'yes' => esc_html__( 'Yes', 'galatia-twitter-feed' )
				)
			...
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__( 'Tweets Cache Time', 'galatia-twitter-feed' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$twitter_api = GalatiaTwitterApi::getInstance();
		...
		if ( $twitter_api->hasUserConnected() ) {
			...
			if ( $response->status ) {
				if ( is_array( $response->data ) && count( $response->data ) ) { ?>
                    <ul class="edgtf-twitter-widget <?php echo esc_attr( $holder_classes ); ?>" <?php echo galatia_edge_get_inline_attrs( $slider_data ); ?>>
						<?php foreach ( $response->data as $tweet ) { ?>

							<?php if ( $type_of_widget == 'slider' ) { ?>

                                <li class="edgtf-tweet-holder">
                                    <div class="edgtf-tweet-text" <?php galatia_edge_inline_style( $twitter_style ); ?>>
			...
			esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'galatia-twitter-feed' );
		}
...
if ( ! function_exists( 'galatia_twitter_widget_load' ) ) {
	function galatia_twitter_widget_load() {
		if ( galatia_twitter_theme_installed() ) {
			register_widget( 'GalatiaTwitterWidget' );
		}
	}

	add_action( 'widgets_init', 'galatia_twitter_widget_load' );
}