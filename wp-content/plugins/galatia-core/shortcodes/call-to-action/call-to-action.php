<?php
namespace GalatiaCore\CPT\Shortcodes\CallToAction;
...
class CallToAction implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Call To Action', 'galatia-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Set Content In Grid', 'galatia-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Content Elements Proportion', 'galatia-core' ),
							'value'      => array(
								esc_html__( '80/20', 'galatia-core' ) => '80',
								esc_html__( '75/25', 'galatia-core' ) => '75',
								esc_html__( '66/33', 'galatia-core' ) => '66',
								esc_html__( '50/50', 'galatia-core' ) => '50'
							),
							'dependency' => array( 'element' => 'layout', 'value' => array( 'normal' ) )
						),
						array(
						...
						array(
							...
							'heading'    => esc_html__( 'Button Top Margin (px)', 'galatia-core' ),
						...
						array(
							'type'       => 'textarea_html',
							'param_name' => 'content',
							'heading'    => esc_html__( 'Content', 'galatia-core' ),
							'value'      => esc_html__( 'I am test text for Call to Action shortcode content', 'galatia-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = galatia_core_get_shortcode_module_template_part( 'templates/call-to-action', 'call-to-action', '', $params );
	...
	private function getButtonHolderStyles( $params ) {
		...
		if ( ! empty( $params['button_top_margin'] ) && $params['layout'] === 'simple' ) {
			$styles[] = 'margin-top: ' . galatia_edge_filter_px( $params['button_top_margin'] ) . 'px';
		}
