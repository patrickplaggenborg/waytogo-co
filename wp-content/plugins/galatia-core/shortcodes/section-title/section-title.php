<?php
namespace GalatiaCore\CPT\Shortcodes\SectionTitle;
...
class SectionTitle implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Section Title', 'galatia-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						...
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Two Columns', 'galatia-core' ) => 'two-columns'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Title - Text Position', 'galatia-core' ),
							'value'       => array(
								esc_html__( 'Title Left - Text Right', 'galatia-core' ) => 'title-left',
								esc_html__( 'Title Right - Text Left', 'galatia-core' ) => 'title-right'
							),
							'save_always' => true,
						...
						array(
							...
							'heading'     => esc_html__( 'Space Between Columns', 'galatia-core' ),
							'value'       => array(
								esc_html__( 'Normal', 'galatia-core' ) => 'normal',
								esc_html__( 'Small', 'galatia-core' )  => 'small',
								esc_html__( 'Tiny', 'galatia-core' )   => 'tiny'
							),
							'save_always' => true,
							'dependency'  => array( 'element' => 'type', 'value' => array( 'two-columns' ) )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'position',
							'heading'     => esc_html__( 'Horizontal Position', 'galatia-core' ),
							'value'       => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Holder Side Padding (px or %)', 'galatia-core' )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'title',
						...
						array(
                            ...
                            'group'       => esc_html__( 'Title Style', 'galatia-core' )
                        ),
                        array(
                            'type'        => 'textfield',
                            'param_name'  => 'title_top_margin',
                            'heading'     => esc_html__( 'Title Top Margin', 'galatia-core' ),
                            'save_always' => true,
                            'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
                            'group'       => esc_html__( 'Title Style', 'galatia-core' )
                        ),
                        array(
                            ...
                            'heading'     => esc_html__( 'Title Bottom Margin', 'galatia-core' ),
                            'save_always' => true,
                            'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
                            'group'       => esc_html__( 'Title Style', 'galatia-core' )
                        ),
						...
						array(
							...
							'heading'     => esc_html__( 'Words with Bold Font Weight', 'galatia-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "bold" font weight. Separate the positions with commas (e.g. if you would like the first, second, and third word to have a light font weight, you would enter "1,2,3")', 'galatia-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'galatia-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Words with Light Font Weight', 'galatia-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "light" font weight. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a light font weight, you would enter "1,3,4")', 'galatia-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'galatia-core' )
						),
						array(
						...
						array(
							...
							'group'       => esc_html__( 'Text Style', 'galatia-core' )
						),
						array(
							'type'       => 'colorpicker',
						...
						array(
							...
							'heading'    => esc_html__( 'Text Font Size (px)', 'galatia-core' ),
							'dependency' => array( 'element' => 'text', 'not_empty' => true ),
							'group'      => esc_html__( 'Text Style', 'galatia-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Text Line Height (px)', 'galatia-core' ),
							'dependency' => array( 'element' => 'text', 'not_empty' => true ),
							'group'      => esc_html__( 'Text Style', 'galatia-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Text Font Weight', 'galatia-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Text Top Margin (px)', 'galatia-core' ),
							'dependency' => array( 'element' => 'text', 'not_empty' => true ),
							'group'      => esc_html__( 'Text Style', 'galatia-core' )
						)
					)
	...
	public function render( $atts, $content = null ) {
		...
		$html = galatia_core_get_shortcode_module_template_part( 'templates/section-title', 'section-title', '', $params );
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . galatia_edge_filter_px($params['title_top_margin']) . 'px';
		}

		if ( ! empty( $params['title_bottom_margin'] ) ) {
			$styles[] = 'margin-bottom: ' . galatia_edge_filter_px($params['title_bottom_margin']) . 'px';
		}
    ...
    private function getModifiedText( $params ) {
        ...
        if ( ! empty( $text ) && ! empty( $params['text_break_words']) ) {
            $split_text = explode( ' ', $text );
            $text_break_words = $params['text_break_words'];
	...
	private function getTextStyles( $params ) {
		...
		if ( ! empty( $params['text_font_size'] ) ) {
			$styles[] = 'font-size: ' . galatia_edge_filter_px( $params['text_font_size'] ) . 'px';
		}
		
		if ( ! empty( $params['text_line_height'] ) ) {
			$styles[] = 'line-height: ' . galatia_edge_filter_px( $params['text_line_height'] ) . 'px';
		}
		...
		if ( $params['text_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . galatia_edge_filter_px( $params['text_margin'] ) . 'px';
		}
