<?php
namespace GalatiaCore\CPT\Shortcodes\Button;
...
class Button implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Button', 'galatia-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'value'       => esc_html__( 'Button Text', 'galatia-core' ),
								'save_always' => true,
								'admin_label' => true
							),
							array(
						...
						galatia_edge_icon_collections()->getVCParamsArray( array(), '', true ),
						array(
							array(
								'type'       => 'colorpicker',
								'param_name' => 'color',
							...
							array(
								...
								'heading'    => esc_html__( 'Hover Color', 'galatia-core' ),
								'group'      => esc_html__( 'Design Options', 'galatia-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Font Size (px)', 'galatia-core' ),
								'group'      => esc_html__( 'Design Options', 'galatia-core' )
							),
							array(
								'type'        => 'dropdown',
							...
							array(
								'type'        => 'textfield',
								'param_name'  => 'padding',
								'heading'     => esc_html__( 'Button Padding', 'galatia-core' ),
								'description' => esc_html__( 'Insert padding in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'galatia-core' ),
								'dependency'  => array( 'element' => 'type', 'value' => array( 'solid', 'outline' ) ),
								'group'       => esc_html__( 'Design Options', 'galatia-core' )
							)
						)
	...
	public function render( $atts, $content = null ) {
		...
		return galatia_core_get_shortcode_module_template_part( 'templates/' . $params['html_type'], 'button', '', $params );
	}
	...
	private function getButtonStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . galatia_edge_filter_px( $params['font_size'] ) . 'px';
		}
