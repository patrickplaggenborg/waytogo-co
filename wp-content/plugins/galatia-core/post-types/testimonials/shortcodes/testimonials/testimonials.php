...
namespace GalatiaCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Testimonials', 'galatia-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
                        ...
                        array(
                            ...
                            'heading'     => esc_html__( 'Intro Title', 'galatia-core' ),
                            'save_always' => true
                        ),
						array(
						    'type'        => 'dropdown',
                            'param_name'  => 'show_title',
                            'heading'     => esc_html__( 'Show Testimonials\' Title', 'galatia-core' ),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'galatia-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'galatia-core' ),
							'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'galatia-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'box_color',
							'heading'    => esc_html__( 'Content Box Color', 'galatia-core' ),
							'dependency' => Array( 'element' => 'type', 'value' => 'boxed' )
						),
						array(
							'type'        => 'dropdown',
	...
	public function render( $atts, $content = null ) {
        ...
        return galatia_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'galatia-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'galatia-core' ) . ': ' . $testimonials_category_title;
				}
