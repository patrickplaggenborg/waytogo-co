...
class PortfolioList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Portfolio List', 'galatia-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Portfolio List Template', 'galatia-core' ),
							'value'       => array(
								esc_html__( 'Gallery', 'galatia-core' ) => 'gallery',
								esc_html__( 'Masonry', 'galatia-core' ) => 'masonry',
                                esc_html__( 'Justified Gallery', 'galatia-core' ) => 'justified-gallery'
							),
							'save_always' => true,
							'admin_label' => true
						),
						...
						array(
							...
							'heading'     => esc_html__( 'Number of Columns', 'galatia-core' ),
							'value'       => array_flip( galatia_edge_get_number_of_columns_array( true ) ),
							'description' => esc_html__( 'Default value is Three', 'galatia-core' ),
						),
                        array(
                            'type'        => 'textfield',
                            'param_name'  => 'space_value',
                            'heading'     => esc_html__( 'Space Between Items', 'galatia-core' ),
                            'description' => esc_html__( 'Space between items for Justified Gallery template', 'galatia-core' ),
                            'dependency' => array('element' => 'type', 'value' => 'justified-gallery'),
                            'save_always' => true
                        ),
                        array(
                            'type'        => 'textfield',
                            'param_name'  => 'row_height',
                            'heading'     => esc_html__( 'Row Height', 'galatia-core' ),
                            'description' => esc_html__( 'Targeted row height, which may vary depending on the proportions of the images.', 'galatia-core' ),
                            'value'       => '',
                            'dependency'  => array( 'element' => 'type', 'value' => array( 'justified-gallery' ) ),
                        ),
						array(
						    'type'        => 'dropdown',
                            'param_name'  => 'lower_even_columns',
                            'heading'     => esc_html__('Lower even columns', 'galatia-core'),
                            'value'       => array_flip(galatia_edge_get_yes_no_select_array(false, false)),
                            'dependency'  => array('element' => 'number_of_columns', 'value' => array('', 'two', 'three', 'four', 'five', 'six'))
                        ),
						array(
							'type'        => 'dropdown',
						...
						array(
                            'type'        => 'dropdown',
                            'param_name'  => 'additional_spacing',
                            'heading'     => esc_html__('Enable additional spacing between list rows', 'galatia-core'),
                            'value'       => array_flip(galatia_edge_get_yes_no_select_array(false, false)),
                            'dependency'  => array('element' => 'space_between_items', 'value' => array('tiny', 'small', 'normal', 'medium', 'large', 'huge'))
                        ),
						array(
							'type'        => 'textfield',
						...
						array(
							...
							'heading'     => esc_html__( 'Enable Fixed Image Proportions', 'galatia-core' ),
							'value'       => array_flip( galatia_edge_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Set predefined image proportions for your masonry portfolio list. This option will apply image proportions you set in Portfolio Single page - dimensions for masonry option.', 'galatia-core' ),
							'dependency'  => array( 'element' => 'type', 'value' => array( 'masonry' ) )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'enable_image_shadow',
							'heading'     => esc_html__( 'Enable Image Shadow', 'galatia-core' ),
							'value'       => array_flip( galatia_edge_get_yes_no_select_array( false ) ),
							'save_always' => true
						),
						array(
						...
						array(
							...
							'heading'    => esc_html__( 'Item Style', 'galatia-core' ),
							'value'      => array(
								esc_html__( 'Standard - Shader', 'galatia-core' )                 => 'standard-shader',
								esc_html__( 'Standard - Switch Featured Images', 'galatia-core' ) => 'standard-switch-images',
								esc_html__( 'Gallery - Overlay', 'galatia-core' )                 => 'gallery-overlay',
								esc_html__( 'Gallery - Slide From Image Bottom', 'galatia-core' ) => 'gallery-slide-from-image-bottom',
                                esc_html__( 'Masonry - Side Text', 'galatia-core' )               => 'masonry-side-text'
							),
							'group'      => esc_html__( 'Content Layout', 'galatia-core' )
						),
						array(
						    'type'  => 'colorpicker',
                            'param_name' => 'gallery_overlay_bg_color',
                            'heading'    => esc_html__('Hover Overlay Background Color', 'galatia-core'),
                            'dependency' => array('element' => 'item_style', 'value' => 'gallery-overlay'),
                            'group'      => esc_html__( 'Content Layout', 'galatia-core' )
                        ),
                        array(
						    'type'  => 'dropdown',
                            'param_name' => 'gallery_info_skin',
                            'heading'    => esc_html__('Hover Overlay Info Skin', 'galatia-core'),
                            'dependency' => array('element' => 'item_style', 'value' => 'gallery-overlay'),
                            'value'      => array(
								esc_html__( 'Dark / Default', 'galatia-core' ) => 'dark',
						...
						array(
						    'type'  => 'dropdown',
                            'param_name' => 'advanced_hover',
                            'heading'    => esc_html__( 'Enable Advanced Hover Effect', 'galatia-core' ),
                            'description'    => esc_html__( 'Enabling this option will enable ripple effect when hovering over portfolio images', 'galatia-core' ),
                            'value'      => array_flip(galatia_edge_get_yes_no_select_array(false, false)),
                            'group'      => esc_html__( 'Content Layout', 'galatia-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Content Top Margin (px or %)', 'galatia-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Content Bottom Margin (px or %)', 'galatia-core' ),
							'dependency' => array( 'element' => 'item_style', 'value' => array( 'standard-shader', 'standard-switch-images' ) ),
							'group'      => esc_html__( 'Content Layout', 'galatia-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Enable Title', 'galatia-core' ),
						...
						array(
                            ...
                            'heading'    => esc_html__('Title Skin', 'galatia-core'),
                            'value'      => array(
                                esc_html__('Light / Default', 'galatia-core') => 'light',
                                esc_html__('Dark', 'galatia-core') => 'dark',
                            ),
                            'dependency' => array('element' => 'item_style', 'value' => 'masonry-side-text'),
                            'group'      => esc_html__( 'Content Layout', 'galatia-core' )
                        ),
						array(
							'type'       => 'dropdown',
						...
						array(
							...
							'heading'    => esc_html__( 'Title Text Transform', 'galatia-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Enable Category', 'galatia-core' ),
							'value'      => array_flip( galatia_edge_get_yes_no_select_array( false, true ) ),
							'group'      => esc_html__( 'Content Layout', 'galatia-core' ),
                            'dependency' => array('element' => 'item_style', 'value' => array('standard-shader','standard-switch-images','gallery-overlay','gallery-slide-from-image-bottom')),
                            'save_always' => true
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'enable_count_images',
							'heading'    => esc_html__( 'Enable Number of Images', 'galatia-core' ),
							'value'      => array_flip( galatia_edge_get_yes_no_select_array( false, true ) ),
							'dependency' => array( 'element' => 'type', 'value' => array( 'gallery' ) ),
							'group'      => esc_html__( 'Content Layout', 'galatia-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Enable Excerpt', 'galatia-core' ),
							'value'      => array_flip( galatia_edge_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Content Layout', 'galatia-core' ),
                            'dependency' => array('element' => 'item_style', 'value' => array('standard-shader','standard-switch-images','gallery-overlay','gallery-slide-from-image-bottom'))
						),
						array(
							...
							'heading'     => esc_html__( 'Excerpt Length', 'galatia-core' ),
							'description' => esc_html__( 'Number of characters', 'galatia-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Pagination Type', 'galatia-core' ),
							'value'      => array(
								esc_html__( 'None', 'galatia-core' )            => 'no-pagination',
								esc_html__( 'Standard', 'galatia-core' )        => 'standard',
								esc_html__( 'Load More', 'galatia-core' )       => 'load-more',
								esc_html__( 'Infinite Scroll', 'galatia-core' ) => 'infinite-scroll'
							),
							'group'      => esc_html__( 'Additional Features', 'galatia-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'load_more_top_margin',
							'heading'    => esc_html__( 'Load More Top Margin (px or %)', 'galatia-core' ),
						...
						array(
							'type'       => 'dropdown',
							'param_name' => 'filter',
							'heading'    => esc_html__( 'Enable Category Filter', 'galatia-core' ),
							'value'      => array_flip( galatia_edge_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Additional Features', 'galatia-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Order By', 'galatia-core' ),
							'value'      => array(
								esc_html__( 'Name', 'galatia-core' )  => 'name',
								esc_html__( 'Count', 'galatia-core' ) => 'count',
								esc_html__( 'Id', 'galatia-core' )    => 'id',
								esc_html__( 'Slug', 'galatia-core' )  => 'slug'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Filter Text Transform', 'galatia-core' ),
							'value'      => array_flip( galatia_edge_get_text_transform_array( true ) ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'galatia-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Bottom Margin (px or %)', 'galatia-core' ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'galatia-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Enable Article Animation', 'galatia-core' ),
							'value'       => array_flip( galatia_edge_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Enabling this option you will enable appears animation for your portfolio list items', 'galatia-core' ),
							'group'       => esc_html__( 'Additional Features', 'galatia-core' )
						),
                        array(
							'type'        => 'dropdown',
							'param_name'  => 'enable_article_parallax',
							'heading'     => esc_html__( 'Enable Article Parallax Effect', 'galatia-core' ),
							'value'       => array_flip( galatia_edge_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Enabling this option you will enable parallax effect for your portfolio list items', 'galatia-core' ),
							'group'       => esc_html__( 'Additional Features', 'galatia-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		$args   = array(
			...
			'space_value'              => '',
			'row_height'               => '',
			'lower_even_columns'       => 'no',
			'space_between_items'      => 'normal',
			'additional_spacing'       => 'no',
			'number_of_items'          => '-1',
			...
			'pagination_position'      => '',
            'gallery_info_skin'        => '',
            'gallery_overlay_bg_color' => '',
            'title_skin'               => 'light',
            'enable_article_parallax'  => 'no'
		);
		...
		$additional_params['holder_data']          = galatia_edge_get_holder_data_for_cpt( $params, $additional_params );
		...
		$html = galatia_core_get_cpt_shortcode_module_template_part( 'portfolio', 'portfolio-list', 'portfolio-holder', $params['type'], $params, $additional_params );
	...
	public function getHolderClasses( $params, $args ) {
		...
		$classes[] = $params['enable_article_parallax'] === 'yes' ? 'edgtf-pl-has-parallax' : '';
		...
		if(!empty($params['lower_even_columns']) && $params['lower_even_columns'] == 'yes'){
		    $classes[] = 'edgtf-with-lower-even-columns';
        }
        if(!empty($params['advanced_hover']) && $params['advanced_hover'] == 'yes'){
		    $classes[] = 'edgtf-pl-with-advanced-hover';
        }
        if(!empty($params['additional_spacing']) && $params['additional_spacing'] == 'yes'){
		    $classes[] = 'edgtf-additional-row-spacing';
        }
        if(!empty($params['gallery_info_skin'])){
		    $classes[] = 'edgtf-pl-info-' . $params['gallery_info_skin'] . '-skin';
        }


        $classes[] = $params['title_skin'] == 'dark' ? 'edgtf-pl-with-dark-title' : '';
	...
	public function getArticleClasses( $params ) {
		...
		if( get_post_meta( get_the_ID(), "edgtf_portfolio_masonry_original_padding_meta", true ) !== "" ){
		    $classes[] = 'edgtf-pl-item-with-custom-padding';
        }
    ...
    public function getArticlePadding( $params ) {
        $styles = array();

        $type       = $params['type'];
        $enable_fixed_proportions = $params['enable_fixed_proportions'];
        $item_padding = get_post_meta( get_the_ID(), "edgtf_portfolio_masonry_original_padding_meta", true );

        if ( $type == 'masonry' && $enable_fixed_proportions == 'no' && ! empty($item_padding)) {
    ...
    public function getTextHolderStyles( $params ){
        ...
        if ( ! empty($params['gallery_overlay_bg_color']) ) {
            $styles[] = 'background-color: ' . $params['gallery_overlay_bg_color'];
        }
		...
		if ( $params['type'] == 'masonry' && $params['enable_fixed_proportions'] === 'yes' ) {
			...
			switch ( $fixed_image_size ) {
				...
				case 'large-width-height':
					$thumb_size = 'galatia_edge_image_huge';
					break;
				default :
					$thumb_size = 'full';
					break;
	...
	public function getStandardContentStyles( $params ) {
		...
		if ( ! empty( $margin_top ) ) {
			if ( galatia_edge_string_ends_with( $margin_top, '%' ) || galatia_edge_string_ends_with( $margin_top, 'px' ) ) {
		...
		if ( ! empty( $margin_bottom ) ) {
			if ( galatia_edge_string_ends_with( $margin_bottom, '%' ) || galatia_edge_string_ends_with( $margin_bottom, 'px' ) ) {
	...
	public function getLoadMoreStyles( $params ) {
		...
		if ( ! empty( $params['load_more_top_margin'] ) ) {
			...
			if ( galatia_edge_string_ends_with( $margin, '%' ) || galatia_edge_string_ends_with( $margin, 'px' ) ) {
				$styles[] = 'margin-top: ' . $margin;
			} else {
				$styles[] = 'margin-top: ' . galatia_edge_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getFilterHolderStyles( $params ) {
		...
		if ( ! empty( $params['filter_bottom_margin'] ) ) {
			...
			if ( galatia_edge_string_ends_with( $margin, '%' ) || galatia_edge_string_ends_with( $margin, 'px' ) ) {
				...
				$styles[] = 'margin-bottom: ' . galatia_edge_filter_px( $margin ) . 'px';
			}
		}
